...
class PriorityQueue:
    ...
    def exist(self, item):
        ...
        for i in range(len(self.heap)): 
            if item == self.heap[i][2][0][0]:       # self.heap[i][2][0] : [state, direction, cost]
                exist = True
                break
        return exist
    ...
    def getItemWithPos(self, pos):
        exist = False
        for i in range(len(self.heap)):
            state = self.heap[i][2][0][0]       # self.heap[i][2][0] : [state, direction, cost]
            if pos == state:
                return self.heap[i][2]
        print('Error. Failed to get pos ' + str(pos) + ' in the queue.')
