...
testheader = ['Map', 'Search Algorithm 1', 'Search Algorithm w', 'Mean Score using S1', 'Mean Score using S2', 'T-score', 'P-value']

with open('ttest_scores_mm.csv', 'w', newline='') as f:
  writer = csv.writer(f)
...
for l in Layout:
  ...
  df1 = df1.rename(columns={'Score': 'mm'})
  col1 = df1['mm'].copy()
  ...
  for s in Search:
    ...
    df2 = df2.rename(columns={'Score': s})
    col2 = df2[s].copy()
    ...
    with open('ttest_scores_mm.csv', 'a', newline='') as f:
      writer = csv.writer(f)
...
with open('ttest_scores_mm0.csv', 'w', newline='') as f:
  writer = csv.writer(f)
  writer.writerow(testheader)
  f.close()
...
for l in Layout:
  ...
  df1 = df1.drop('index', axis=1)
  df1 = df1.drop('Search Algorithm', axis=1)
  df1 = df1.rename(columns={'Score': 'mm0'})
  col1 = df1['mm0'].copy()
  ...
  for s in Search:
    ...
    with open('ttest_scores_mm0.csv', 'a', newline='') as f:
      writer = csv.writer(f)
      writer.writerow(data_items)
      data_items = []
      f.close()
...
with open('ttest_scores_mm_mm0.csv', 'w', newline='') as f:
  writer = csv.writer(f)
  writer.writerow(testheader)
  f.close()
...
for l in Layout:
  ...
  df2 = df2.drop('index', axis=1)
  df2 = df2.drop('Search Algorithm', axis=1)
  df2 = df2.rename(columns={'Score': 'mm0'})
  col2 = df2['mm0'].copy()
  ...
  with open('ttest_scores_mm_mm0.csv', 'a', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(data_items)
    data_items = []
    f.close()
