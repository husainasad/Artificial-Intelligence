...
testheader = ['Map', 'Search Algorithm 1', 'Search Algorithm w', 'Mean Path Cost using S1', 'Mean Path Cost using S2', 'T-score', 'P-value']

with open('ttest_costs_mm.csv', 'w', newline='') as f:
  writer = csv.writer(f)
...
for l in Layout:
  ...
  for s in Search:
    ...
    df2 = df2.rename(columns={'Total Cost': s})
    col2 = df2[s].copy()
    ...
    with open('ttest_costs_mm.csv', 'a', newline='') as f:
      writer = csv.writer(f)
...
with open('ttest_costs_mm0.csv', 'w', newline='') as f:
  writer = csv.writer(f)
  writer.writerow(testheader)
  f.close()
...
for l in Layout:
  ...
  df1 = df1.drop('index', axis=1)
  df1 = df1.drop('Search Algorithm', axis=1)
  df1 = df1.rename(columns={'Total Cost': 'mm0'})
  col1 = df1['mm0'].copy()
  ...
  for s in Search:
    ...
    with open('ttest_costs_mm0.csv', 'a', newline='') as f:
      writer = csv.writer(f)
      writer.writerow(data_items)
      data_items = []
      f.close()
...
with open('ttest_costs_mm_mm0.csv', 'w', newline='') as f:
  writer = csv.writer(f)
  writer.writerow(testheader)
  f.close()
...
for l in Layout:
  ...
  df2 = df2.drop('index', axis=1)
  df2 = df2.drop('Search Algorithm', axis=1)
  df2 = df2.rename(columns={'Total Cost': 'mm0'})
  col2 = df2['mm0'].copy()
  ...
  with open('ttest_costs_mm_mm0.csv', 'a', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(data_items)
    data_items = []
    f.close()
