...
def axiom_generator_percept_sentence(t, tvec):
    ...
    perceptual_fluents = [percept_stench_str(t), percept_breeze_str(t), percept_glitter_str(t), percept_bump_str(t), percept_scream_str(t)]
    perceptStatus=[]
    for i in range(len(tvec)):
        if(tvec[i]==False):
            perceptStatus.append('~'+perceptual_fluents[i])
        else:
            perceptStatus.append(perceptual_fluents[i])
    axiom_str+=' & '.join(str(i) for i in perceptStatus)
    return axiom_str
...
def axiom_generator_initial_location_assertions(x, y):
    ...
    # Comment or delete the next line once this function has been implemented.
    # utils.print_not_implemented()
    # ~pit_str(x, y) & ~wumpus_str(x, y)
    initLoc=[]
    initLoc.append('~'+pit_str(x, y))
    initLoc.append('~'+wumpus_str(x, y))
    axiom_str+=' & '.join(str(i) for i in initLoc)
    return axiom_str
...
def axiom_generator_pits_and_breezes(x, y, xmin, xmax, ymin, ymax):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    # breeze_str(x, y) <=> pit_str(x, y) | pit_str(x-1, y) | pit_str(x+1, y) | pit_str(x, y-1) | pit_str(x, y+1)
    breezeLoc=[]
    breezeLoc.append(breeze_str(x, y))
    breezeLoc.append(' <=> (')
    axiom_str=''.join(str(i) for i in breezeLoc)
    breezeLoc=[]
    adjacentSquares=[(x, y),(x+1, y),(x-1, y),(x, y+1),(x, y-1)]
    for i in adjacentSquares:
        if i[0]>=xmin and i[0]<=xmax and i[1]>=ymin and i[1]<=ymax:
            breezeLoc.append(pit_str(i[0], i[1]))
    axiom_str+=' | '.join(str(i) for i in breezeLoc)
    axiom_str+=')'
    return axiom_str
...
def axiom_generator_wumpus_and_stench(x, y, xmin, xmax, ymin, ymax):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    # stench_str(x, y) <=> wumpus_str(x, y) | wumpus_str(x-1, y) | wumpus_str(x+1, y) | wumpus_str(x, y-1) | wumpus_str(x, y+1)
    stenchLoc=[]
    stenchLoc.append(stench_str(x, y))
    stenchLoc.append(' <=> (')
    axiom_str=''.join(str(i) for i in stenchLoc)
    stenchLoc=[]
    adjacentSquares=[(x, y),(x+1, y),(x-1, y),(x, y+1),(x, y-1)]
    for i in adjacentSquares:
        if i[0]>=xmin and i[0]<=xmax and i[1]>=ymin and i[1]<=ymax:
            stenchLoc.append(wumpus_str(i[0], i[1]))
    axiom_str+=' | '.join(str(i) for i in stenchLoc)
    axiom_str+=')'
    return axiom_str
...
def axiom_generator_at_least_one_wumpus(xmin, xmax, ymin, ymax):
    ...
    # Comment or delete the next line once this function has been implemented.
    # utils.print_not_implemented()
    # wumpus_str(0, 0) | wumpus_str(0, 1) | ..... for 2d matrix
    wumpusLoc=[]
    for i in range(xmin, xmax+1):
        for j in range(ymin, ymax+1):
            wumpusLoc.append(wumpus_str(i, j))
    axiom_str+=' | '.join(str(i) for i in wumpusLoc)
    return axiom_str
...
def axiom_generator_at_most_one_wumpus(xmin, xmax, ymin, ymax):
    ...
    # Comment or delete the next line once this function has been implemented.
    # utils.print_not_implemented()
    # for 2d matrix: wumpus_str(x1,y1) => ~wumpus_str(x2, y2) if (x2,y2)!=(x1,y1)
    ...
    #         wumpus+='('
    #         wumpus_present=[]
    #         wumpus_present.append(wumpus_str(i, j))
    #         for k in range(xmin, xmax+1):
    #             for l in range(ymin, ymax+1):
    #                 if (k!=i) or (l!=j):
    #                     wumpus_present.append('~'+ wumpus_str(k, l))
    #         wumpus+=' & '.join(str(x) for x in wumpus_present)
    #         wumpus+=')'
    #         wumpusLoc.append(wumpus)
    # axiom_str+=' | '.join(str(i) for i in wumpusLoc)
    # return axiom_str
    ...
    for i in range(xmin, xmax+1):
        for j in range(ymin, ymax+1):
            wumpus_present='('+wumpus_str(i, j)+' >> ('
            wumpus_absent=[]
            for k in range(xmin, xmax+1):
                for l in range(ymin, ymax+1):
                    if (k!=i) or (l!=j):
                        wumpus_absent.append('~'+ wumpus_str(k, l))
            wumpus_present+=' & '.join(str(x) for x in wumpus_absent)
            wumpus_present+='))'
            wumpusLoc.append(wumpus_present)
    axiom_str+=' & '.join(str(i) for i in wumpusLoc)
    return axiom_str
...
def axiom_generator_only_in_one_location(xi, yi, xmin, xmax, ymin, ymax, t = 0):
    ...
    # Comment or delete the next line once this function has been implemented.
    # utils.print_not_implemented()
    # for 2d matrix, if (i,j)!=(xi,yi), append ~state_loc_str(i, j, t) else append state_loc_str(i, j, t)
    agentLoc=[]
    for i in range(xmin, xmax+1):
        for j in range(ymin, ymax+1):
            if (i==xi) and (j==yi):
                agentLoc.append(state_loc_str(i, j, t))
                break
    ...
    for i in range(xmin, xmax+1):
        for j in range(ymin, ymax+1):
            if (i!=xi) or (j!=yi):
                agentLoc.append('~'+state_loc_str(i, j, t))
    axiom_str+=' & '.join(str(i) for i in agentLoc)
    return axiom_str
...
def axiom_generator_only_one_heading(heading = 'north', t = 0):
    ...
    # Comment or delete the next line once this function has been implemented.
    # utils.print_not_implemented()
    state_fluents = [state_heading_north_str(t), state_heading_east_str(t), state_heading_south_str(t), state_heading_west_str(t)]
    state_fluents_val = ['north', 'east', 'south', 'west']

    agentDir=[]
    for i in range(len(state_fluents_val)):
        if state_fluents_val[i]==heading:
            agentDir.append(state_fluents[i])
            break
    ...
    axiom_str+=' & '.join(str(i) for i in agentDir)
    return axiom_str
...
def axiom_generator_have_arrow_and_wumpus_alive(t = 0):
    ...
    # utils.print_not_implemented()
    status=[]
    status.append(state_have_arrow_str(t))
    status.append(state_wumpus_alive_str(t))
...
def axiom_generator_location_OK(x, y, t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    # location is safe if there is not pit and no wumpus OR wumpus is at location but wumpus is dead
    axiom_str+=state_OK_str(x,y,t)
    axiom_str+=' <=> ('
    safe_loc=[]
    safe_loc.append('~'+pit_str(x, y) + ' & (~'+wumpus_str(x, y)+ ' | ('+wumpus_str(x, y)+ ' & ~'+state_wumpus_alive_str(t)+'))')
    axiom_str+=''.join(str(i) for i in safe_loc)
    axiom_str+=')'
    return axiom_str
...
def axiom_generator_breeze_percept_and_location_property(x, y, t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    # if agent is at location (x,y,t), this implies that if breeze is detected, then location has a breeze (and vice versa)
    stateStatus=[]
    breezeStatus=[]
    stateStatus.append(state_loc_str(x,y,t)+ ' >> ')
    breezeStatus.append('('+percept_breeze_str(t)+' <=> '+breeze_str(x,y)+')')
    status=stateStatus+ breezeStatus
    axiom_str+=''.join(str(i) for i in status)
    return axiom_str
...
def axiom_generator_stench_percept_and_location_property(x, y, t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    # if agent is at location (x,y,t), this implies that if stench is detected, then location has a stench (and vice versa)
    stateStatus=[]
    stenchStatus=[]
    stateStatus.append(state_loc_str(x,y,t)+ ' >> ')
    stenchStatus.append('('+percept_stench_str(t)+' <=> '+stench_str(x,y)+')')
    status=stateStatus+ stenchStatus
    axiom_str+=''.join(str(i) for i in status)
    return axiom_str
...
def axiom_generator_at_location_ssa(t, x, y, xmin, xmax, ymin, ymax):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    # agent at (x,y) at t+1 <=> agent at (x,y) & (~forward(t)|bump(t+1)| grab(t)| shoot(t)|turnleft(t)|turnright(t)|wait(t)); for other locations agent should heading in right direction and move forward at (t) 
    status_new_t=[]
    status_new_t.append(state_loc_str(x, y, t+1)+ ' <=> (')
    status_t=[]
    # status_t.append('('+ state_loc_str(x, y, t)+ ' & ~'+action_forward_str(t)+ ' & ('+action_grab_str(t)+' | '+action_shoot_str(t)+' | '+action_turn_left_str(t)+' | '+action_turn_right_str(t)+' | '+action_wait_str(t)+' | '+percept_bump_str(t+1)+'))')
    status_t.append('('+ state_loc_str(x, y, t)+ ' & ~'+action_forward_str(t)+')')
    
    # if ((x-1>=xmin) and (x-1<=xmax) and (y>=ymin) and (y<=ymax)):
    #     status_t.append(' | ('+state_loc_str(x-1, y, t)+ ' & '+ state_heading_east_str(t)+' & '+ action_forward_str(t)+')')

    # if ((x+1>=xmin) and (x+1<=xmax) and (y>=ymin) and (y<=ymax)):
    #     status_t.append(' | ('+state_loc_str(x+1, y, t)+ ' & '+ state_heading_west_str(t)+' & '+ action_forward_str(t)+')')

    # if ((x>=xmin) and (x<=xmax) and (y-1>=ymin) and (y-1<=ymax)):
    #     status_t.append(' | ('+state_loc_str(x, y-1, t)+ ' & '+ state_heading_north_str(t)+' & '+ action_forward_str(t)+')')

    # if ((x>=xmin) and (x<=xmax) and (y+1>=ymin) and (y+1<=ymax)):
    #     status_t.append(' | ('+state_loc_str(x, y+1, t)+ ' & '+ state_heading_south_str(t)+' & '+ action_forward_str(t)+')')

    adjacentSquares=[(x+1, y),(x-1, y),(x, y+1),(x, y-1)]
    state_fluents = [state_heading_west_str(t), state_heading_east_str(t), state_heading_south_str(t), state_heading_north_str(t)]

    for i in range(len(adjacentSquares)):
        if adjacentSquares[i][0]>=xmin and adjacentSquares[i][0]<=xmax and adjacentSquares[i][1]>=ymin and adjacentSquares[i][1]<=ymax:
            status_t.append(' | ('+state_loc_str(adjacentSquares[i][0], adjacentSquares[i][1], t)+ ' & '+ state_fluents[i]+' & '+ action_forward_str(t)+')')

    status=status_new_t+status_t
    axiom_str+=''.join(str(i) for i in status)
    axiom_str+=')'
    return axiom_str
...
def axiom_generator_have_arrow_ssa(t):
    ...
    # Comment or delete the next line once this function has been implemented.
    # utils.print_not_implemented()
    # agent will have arrow at (t+1) if it had arrow at (t) and did not shoot arrow at (t)
    status_new_t=[]
    status_new_t.append(state_have_arrow_str(t+1)+' <=> (')
    status_t=[]
    status_t.append(state_have_arrow_str(t)+' & ~'+action_shoot_str(t))
    status=status_new_t+status_t
    axiom_str+=''.join(str(i) for i in status)
    axiom_str+=')'
    return axiom_str
...
def axiom_generator_wumpus_alive_ssa(t):
    ...
    # Comment or delete the next line once this function has been implemented.
    # utils.print_not_implemented()
    # wumpus will be alive at (t+1) if wumpus was alive at (t) and scream is not heard at (t+1)
    status_new_t=[]
    status_new_t.append(state_wumpus_alive_str(t+1)+' <=> (')
    status_t=[]
    status_t.append(state_wumpus_alive_str(t)+' & ~'+percept_scream_str(t+1))
    status=status_new_t+status_t
    axiom_str+=''.join(str(i) for i in status)
    axiom_str+=')'
    return axiom_str
...
def axiom_generator_heading_north_ssa(t):
    ...
    # Comment or delete the next line once this function has been implemented.
    # utils.print_not_implemented()
    # for agent to head north at (t+1), it should have been heading north without turning left or right at (t), or turn left from head east or turn right from head west at time (t)
    status_new_t=[]
    status_new_t.append(state_heading_north_str(t+1)+' <=> (')
    status_t=[]
    status_t.append('('+state_heading_north_str(t)+ ' & ~'+action_turn_left_str(t)+ ' & ~'+action_turn_right_str(t)+') | ('+state_heading_west_str(t)+ ' & '+ action_turn_right_str(t)+') | ('+state_heading_east_str(t)+ ' & '+action_turn_left_str(t)+')')
    status=status_new_t+status_t
    axiom_str+=''.join(str(i) for i in status)
    axiom_str+=')'
    return axiom_str
...
def axiom_generator_heading_east_ssa(t):
    ...
    # Comment or delete the next line once this function has been implemented.
    # utils.print_not_implemented()
    # same has head north but with some modifications
    status_new_t=[]
    status_new_t.append(state_heading_east_str(t+1)+' <=> (')
    status_t=[]
    status_t.append('('+state_heading_east_str(t)+ ' & ~'+action_turn_left_str(t)+ ' & ~'+action_turn_right_str(t)+') | ('+state_heading_north_str(t)+ ' & '+ action_turn_right_str(t)+') | ('+state_heading_south_str(t)+ ' & '+action_turn_left_str(t)+')')
    status=status_new_t+status_t
    axiom_str+=''.join(str(i) for i in status)
    axiom_str+=')'
    return axiom_str
...
def axiom_generator_heading_south_ssa(t):
    ...
    # same has head north but with some modifications
    status_new_t=[]
    status_new_t.append(state_heading_south_str(t+1)+' <=> (')
    status_t=[]
    status_t.append('('+state_heading_south_str(t)+ ' & ~'+action_turn_left_str(t)+ ' & ~'+action_turn_right_str(t)+') | ('+state_heading_east_str(t)+ ' & '+ action_turn_right_str(t)+') | ('+state_heading_west_str(t)+ ' & '+action_turn_left_str(t)+')')
    status=status_new_t+status_t
    axiom_str+=''.join(str(i) for i in status)
    axiom_str+=')'
    return axiom_str
...
def axiom_generator_heading_west_ssa(t):
    ...
    # same has head north but with some modifications
    status_new_t=[]
    status_new_t.append(state_heading_west_str(t+1)+' <=> (')
    status_t=[]
    status_t.append('('+state_heading_west_str(t)+ ' & ~'+action_turn_left_str(t)+ ' & ~'+action_turn_right_str(t)+') | ('+state_heading_south_str(t)+ ' & '+ action_turn_right_str(t)+') | ('+state_heading_north_str(t)+ ' & '+action_turn_left_str(t)+')')
    status=status_new_t+status_t
    axiom_str+=''.join(str(i) for i in status)
    axiom_str+=')'
    return axiom_str
...
def axiom_generator_heading_only_north(t):
    ...
    # Comment or delete the next line once this function has been implemented.
    # utils.print_not_implemented()
    # when heading is north, heading west, east and south should be false
    status=[]
    status_dir=''
    status_dir+=state_heading_north_str(t)+ ' <=> ('
    status_not_dir=[]

    state_fluents = [state_heading_west_str(t), state_heading_east_str(t), state_heading_south_str(t)]

    for i in range(len(state_fluents)):
        status_not_dir.append('~'+state_fluents[i])
    status_dir+=' & '.join(str(i) for i in status_not_dir)
    status_dir+=')'
    status.append(status_dir)
    axiom_str+=''.join(str(i) for i in status)
    return axiom_str
...
def axiom_generator_heading_only_east(t):
    ...
    # Comment or delete the next line once this function has been implemented.
    # utils.print_not_implemented()
    # same as only north, but with some modifications
    status=[]
    status_dir=''
    status_dir+=state_heading_east_str(t)+ ' <=> ('
    status_not_dir=[]

    state_fluents = [state_heading_north_str(t), state_heading_west_str(t), state_heading_south_str(t)]
...
def axiom_generator_heading_only_south(t):
    ...
    status=[]
    status_dir=''
    status_dir+=state_heading_south_str(t)+ ' <=> ('
    status_not_dir=[]

    state_fluents = [state_heading_north_str(t), state_heading_west_str(t), state_heading_east_str(t)]
...
def axiom_generator_heading_only_west(t):
    ...
    status=[]
    status_dir=''
    status_dir+=state_heading_west_str(t)+ ' <=> ('
    status_not_dir=[]

    state_fluents = [state_heading_north_str(t), state_heading_east_str(t), state_heading_south_str(t)]
...
def axiom_generator_only_one_action_axioms(t):
    ...
    # Comment or delete the next line once this function has been implemented.
    # utils.print_not_implemented()
    # when one action in array is true, all other actions must be false....use nest loop to iterate through actions

    # result.append('('+action_forward_str(t)+' <=> '+'(~'+action_grab_str(t)+' & '+'~'+action_shoot_str(t)+ ' & '+'~'+action_climb_str(t)+' & '+'~'+action_turn_left_str(t)+' & '+'~'+action_turn_right_str(t)+' & '+'~'+action_wait_str(t)+'))')
    # result.append('('+action_grab_str(t)+' <=> '+'(~'+action_forward_str(t)+' & '+'~'+action_shoot_str(t)+ ' & '+'~'+action_climb_str(t)+' & '+'~'+action_turn_left_str(t)+' & '+'~'+action_turn_right_str(t)+' & '+'~'+action_wait_str(t)+'))')
    # result.append('('+action_shoot_str(t)+' <=> '+'(~'+action_grab_str(t)+' & '+'~'+action_forward_str(t)+ ' & '+'~'+action_climb_str(t)+' & '+'~'+action_turn_left_str(t)+' & '+'~'+action_turn_right_str(t)+' & '+'~'+action_wait_str(t)+'))')
    # result.append('('+action_climb_str(t)+' <=> '+'(~'+action_grab_str(t)+' & '+'~'+action_shoot_str(t)+ ' & '+'~'+action_forward_str(t)+' & '+'~'+action_turn_left_str(t)+' & '+'~'+action_turn_right_str(t)+' & '+'~'+action_wait_str(t)+'))')
    # result.append('('+action_turn_left_str(t)+' <=> '+'(~'+action_grab_str(t)+' & '+'~'+action_shoot_str(t)+ ' & '+'~'+action_climb_str(t)+' & '+'~'+action_forward_str(t)+' & '+'~'+action_turn_right_str(t)+' & '+'~'+action_wait_str(t)+'))')
    # result.append('('+action_turn_right_str(t)+' <=> '+'(~'+action_grab_str(t)+' & '+'~'+action_shoot_str(t)+ ' & '+'~'+action_climb_str(t)+' & '+'~'+action_turn_left_str(t)+' & '+'~'+action_forward_str(t)+' & '+'~'+action_wait_str(t)+'))')
    # result.append('('+action_wait_str(t)+' <=> '+'(~'+action_grab_str(t)+' & '+'~'+action_shoot_str(t)+ ' & '+'~'+action_climb_str(t)+' & '+'~'+action_turn_left_str(t)+' & '+'~'+action_turn_right_str(t)+' & '+'~'+action_forward_str(t)+'))')
    # axiom_str+=' & '.join(str(i) for i in result)
    # return axiom_str

    bases_actions = [action_forward_str(t), action_grab_str(t), action_shoot_str(t), action_climb_str(t), action_turn_left_str(t), action_turn_right_str(t), action_wait_str(t)]

    actions=[]
    action_select=''
    for i in range(len(bases_actions)):
        action_select=''
        action_select+='('+bases_actions[i]+ ' <=> ('
        action_f_select=[]
        for j in range(len(bases_actions)):
            if (j!=i):
                action_f_select.append('~'+bases_actions[j])
        action_select+=' & '.join(str(x) for x in action_f_select)
        action_select+='))'
        actions.append(action_select)
    axiom_str+=' & '.join(str(x) for x in actions)
    return axiom_str
